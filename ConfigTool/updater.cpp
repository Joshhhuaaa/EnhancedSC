...
void CheckForUpdates()
{
    if (!iTargetGame)
    {
        wxLogError("Update Checker: Unable to detect valid Splinter Cell game in parent folder.");
        return;
    }

    std::filesystem::path base = std::filesystem::path(wxGetCwd().ToStdWstring());

    // Store cache in scripts folder alongside the ASI (same as the ASI does)
    std::filesystem::path cacheFilePath = base / (sFixName + "_version_check.bin");
