/******************************************************************************
 ...
 Description:   Quick Inventory HUD Xbox


 Reference:     -

******************************************************************************/
class EQInvHUDXbox extends Actor;
...
function DrawHandItem(ECanvas Canvas, int yPos, bool bDisplayState)
{
    ...
    if(bDisplayState && CurrentCategory == -1)
    {
		Canvas.Style = ERenderStyle.STY_Alpha;
...
{
    local int xPos, yPos, xLightPos, yLightPos, i, nbItems, height;

    if(CurrentCategory == -1)
		return;

    nbItems = PCInventory.GetNbItemInCategory(GetCategory(CurrentCategory));
    if(nbItems == 0)
		return;

    xPos = SCREEN_END_X - eGame.HUD_OFFSET_X - FIRSTCAT_OFFSET - (CurrentCategory + 1) * ITEMBOX_WIDTH_L - CurrentCategory * SPACE_BETWEEN_CAT;
    yPos = SCREEN_END_Y - eGame.HUD_OFFSET_Y - ITEMBOX_HEIGHT_L - (TEXTBOX_HEIGHT + SPACE_BETWEEN_BOX) - SPACE_EXTRA_GOAL; 
    ...
    {      
        ...
        if(i-1 == CurrentItem)
        {
            ...
            DrawItem(Canvas, xPos + 5,  yPos - ITEMBOX_HEIGHT_CAT * i - SEPARATOR_HEIGTHT * i - 4, PCInventory.GetItemInCategory(GetCategory(CurrentCategory), i), true);			
        }
        else
		{			
            DrawItem(Canvas, xPos + 5,  yPos - ITEMBOX_HEIGHT_CAT * i - SEPARATOR_HEIGTHT * i - 4, PCInventory.GetItemInCategory(GetCategory(CurrentCategory), i), false);		
		}
		Canvas.Style = ERenderStyle.STY_Normal;
    }
...
function DrawAnimText(ECanvas Canvas, int xPos, int yPos, FLOAT xLen, FLOAT yLen)
{
    ...
    //Canvas.DrawLine(xPos, yPos, 5, 15, Canvas.black, -1, eLevel.TGAME);    
}
